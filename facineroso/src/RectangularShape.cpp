...
RectangularShape::RectangularShape(glm::ivec2 position, int size, glm::u8vec4 color)
    : Box(position, RectangularGeometry{size, size}, std::make_unique<BoxDrawablePrimitiveSDL>()),
      m_pimpl{std::unique_ptr<Impl, void (*)(Impl *)>(new RectangularShape::Impl, [](Impl *impl) {
          if (impl)
          {
              delete impl;
          }
